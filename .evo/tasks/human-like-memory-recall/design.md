# 设计：类人激活式记忆召回

## 背景与目标

当前 `MemoryRetriever` 在聊天热路径中遍历全部持久记忆并逐条打分；用户请求的偏好类型又覆盖大多数长期记忆，因此候选过滤较宽。Embedding 生产组装尚未接入，现有 SQLite 向量搜索仍是全量余弦计算。记忆关系图只在直接候选排序后扩展前三个节点的一跳邻居，标签共现图尚未参与召回；两张图没有成为定位和联想记忆的主要路径。

本次设计将召回改为类人激活式模型：近期和当前高激活记忆可以直接进入意识，长期记忆通过 HNSW 获得少量语义种子，再由标签图和记忆关系图传播联想，最后依据访问历史、时间、情绪、性格和当前线索选出有限记忆。聊天热路径不得全量扫描持久记忆，不调用大模型，召回成本应主要由固定候选预算决定，而不是随记忆总量线性增长。

本次不改动渲染系统，不追求完整复刻认知科学模型，也不让 HNSW 或图谱取代 SQLite 的权威数据地位。交互采集和首次持久化不得依赖模型；大模型只在 Daydream 阶段整理长期记忆和判断少量复杂关系，不参与在线召回排序。

## 方案

### 1. 总体架构

召回采用“短期激活 + 多路种子 + 图谱传播 + 统一精排”的固定流水线：

```text
当前会话
  -> 本地线索提取
  -> 近期激活池 / Hippocampus 工作集扫描 / HNSW / 关键词与标签索引
  -> 合并、过滤、初始激活计算
  -> 标签图和记忆关系图的有预算传播
  -> ACT-R 启发式精排与人格化探索
  -> 最多 8 条、约 800 tokens 的相关记忆
  -> Runtime Context
```

新增 `MemoryCueExtractor`、`ActiveMemoryPool`、`HnswEmbeddingIndex`、`AssociativeActivationEngine` 和 `MemoryIndexWorker` 五个边界清晰的组件。`MemoryRetriever` 保留统一入口，但改为编排这些组件，不再自行扫描 `MemoryStore::all()`。召回返回每条记忆的来源、分数组成、传播路径和是否为探索候选，供测试、观测和 Prompt 格式化使用。

### 1.5. 核心术语

- **Hippocampus 分区**：SQLite 中的完整待巩固队列，是可靠持久存储，容量无硬上限；积压不丢数据。
- **Hippocampus 工作集**：内存中的有界扫描视图（默认 200 条），从分区按新近度和优先级加载；在线召回只扫描工作集，不触碰分区全量数据。
- **交互轮次**：一次完整的 user input → assistant response → 同步工具调用（可选）周期，对应一个 Hippocampus 经历节点。
- **轮次完成**：assistant 消息已发布、所有同步工具调用已返回、且副作用队列已接受该轮的持久化任务；异步工具的延迟结果作为独立子事件追加，不阻塞轮次提交。

### 2. 在线采集与后台初步整理

每次用户、助手和工具交互都先产生可追溯的原始经历。以一次完整交互轮次作为一个经历节点，用户输入、助手回复、工具调用与工具结果作为该轮次的子事件保存，不将每句话直接建成独立长期记忆。采集采用两阶段：轮次开始时交互线程只提交不可变的采集事件；轮次完成（以现有 `ChatSideEffectQueue` 的 session barrier 且所有同步工具已返回为判定点）后，后台持久化队列把完整轮次写入 `Hippocampus`，同时把对应节点加入 `ActiveMemoryPool`，因此新经历无需等待 Daydream 就能参与后续会话召回。异步工具（如 MCP）的延迟结果作为独立子事件追加到既有轮次节点，不阻塞轮次提交。提醒、工具结果等类型明确的结构化信息可以由确定性代码直接写入对应分区，但仍保留轮次节点和原始子事件作为来源证据。

首次采集不调用远程模型。后台本地整理只补充时间、会话、情绪、来源、因果链和确定性系统标签，建立同轮问答、相邻会话、工具调用与结果等可验证的临时关系。它不得提前做语义合并、冲突判断或自由生成语义标签；这些需要理解上下文的工作统一交给 Daydream。所有后台任务不得阻塞 GUI，失败后可从持久采集事件幂等重放。

`Hippocampus` 是可靠的待巩固存储，`ActiveMemoryPool` 是有容量和衰减的当前激活视图，两者不能互相替代。Daydream 长时间没有发生时，记忆可以退出激活池但仍留在 `Hippocampus`，不得因为激活衰减而丢失待处理经历。

### 3. 记忆生命周期与索引边界

`Working`、`ShortTerm` 和处于 `Hippocampus` 分区的记忆不进入 HNSW。`ActiveMemoryPool` 优先提供当前高激活项；未在激活池中的待巩固经历，从内存中的有界 Hippocampus 工作集执行文本、标签和元数据线性扫描，禁止每轮重新加载 SQLite 全库。Hippocampus 默认工作集目标为 200 条；在该规模下不建第二套 ANN。积压超过目标时不得丢弃持久化经历：内存工作集仍限制为 200 条最新/高优先级项，其余保留在 SQLite 分区中不参与每轮召回，同时触发补充巩固消化积压。若基准测试显示该扫描 P95 超过 10ms（不含查询向量生成），或未来将工作集提高到 500 条以上，再引入独立的短期向量索引。

Daydream 将待巩固经历整理为 Active 的长期记忆后，才计算 Embedding、写入 SQLite 向量表并增量插入 HNSW。这样短命、重复和尚未整理的经历不会污染长期语义索引。

HNSW 只包含 Active、非 Sensitive 的长期记忆。长期记忆内容发生语义变化时执行“旧向量标记删除 + 新向量插入”；状态变为 Archived、Superseded、Deleted、Expired 或 Sensitive 时标记删除。恢复为可召回状态时重新插入。SQLite 中的 `memory_items` 和 `memory_embeddings` 始终是权威数据，HNSW 是可丢弃、可重建的派生索引。新巩固或更新的长期记忆在索引任务完成前暂留 `ActiveMemoryPool`，避免 SQLite 已提交但 HNSW 尚未可见时出现召回空窗。

### 4. HNSW 实现与持久化

使用 `hnswlib` 新增 `HnswEmbeddingIndex`，实现现有 `EmbeddingIndex` 接口。默认距离为 cosine，初始参数（v1.0）为 `M=16`、`efConstruction=200`、`efSearch=50`；查询默认返回 32 个语义候选。参数进入配置并写入索引元数据，后续通过基准测试调整 `efSearch` 与重建触发阈值。

索引文件使用 `memory_hnsw_<model-hash>.bin`，旁路元数据保存模型名称与版本、内容哈希规则、向量维度、距离类型、HNSW 参数、索引 generation 和构建时间。SQLite 新增稳定的数值 label 与 `memoryId` 映射，避免把字符串哈希直接当 label 产生碰撞。模型、维度、建图参数 `M/efConstruction`、距离类型或 generation 不匹配，文件损坏，或索引计数与权威表不一致时，从 SQLite 向量表后台重建；`efSearch` 是运行时查询参数，变化不触发重建。

索引写操作由单个 `MemoryIndexWorker` 串行执行，查询使用读锁或只读快照，禁止 GUI 线程执行模型推理、索引重建和批量向量写入。删除墓碑比例达到 30% 时，由空闲期或 Sleep Cycle 重建索引，消除长期增删改造成的导航质量下降；重建成本不低，不因少量删除频繁触发。

### 5. 一致性与故障恢复

SQLite 新增 `memory_index_jobs` 作为事务型 outbox，记录 `memory_id`、操作类型、模型版本、内容哈希、状态、尝试次数和创建时间。长期记忆变更与索引任务在同一个 SQLite 事务中提交；事务完成后，`MemoryIndexWorker` 幂等消费 `upsert`、`delete` 和 `rebuild` 任务。对于 upsert，Worker 校验内容哈希、生成向量并更新 `memory_embeddings`，随后更新内存中的 HNSW。

HNSW 文件无法参与 SQLite 事务，因此 Worker 合并一批任务后先将完整索引写入临时文件并原子替换正式文件，再更新索引 generation，最后在 SQLite 事务中标记这一批任务完成。程序在任一步异常退出都可以安全重放；同一 `memoryId + model + contentHash` 的重复 upsert 不产生重复有效节点。索引不可用或正在重建时，召回继续使用近期激活池和关键词/标签索引，不回退到聊天热路径的全量扫描。

### 6. 本地线索提取与多路种子

`MemoryCueExtractor` 只做本地计算：规范化用户文本，提取拉丁词元、中文二元/三元片段、已知标签和实体，并读取当前会话主题、未完成目标、情绪快照及 `PersonaProjection::behaviorParameters`。查询文本只计算一次 Embedding，然后交给 HNSW。在线召回禁止为线索提取调用大模型。

关键词和标签使用增量维护的倒排索引；其中标签继续落在现有 `memory_tags`，中文短语索引是可重建的派生结构。默认候选预算为：近期激活池最多 12 条、Hippocampus 工作集扫描最多产生 8 条、HNSW 最多 32 条、关键词/标签最多 12 条；去重和隐私/状态过滤后保留最多 16 个初始种子。某一路不足时不强行填满，也不通过扩大到长期全库补齐。

### 7. 激活模型与访问强化

最终排序采用 ACT-R 启发式而非完整 ACT-R 仿真。对候选记忆 `i` 计算：

```text
A_i = 1.0 * B_i
    + 1.0 * C_i
    + 1.5 * R_i
    + 0.3 * E_i
    + 0.6 * G_i

B_i = 1.0 * strength
    + 1.0 * importance
    + ln(1 + sum((1 + accessAgeHours)^-0.5))

C_i = 2.0 * semanticCue + 1.2 * lexicalOrTagCue
R_i = runtimeActivation
E_i = emotionMatch * currentIntensity * memoryEmotionConfidence
G_i = graphPropagation
```

五个顶层系数分别对应 `baseLevelWeight=1.0`、`cueMatchWeight=1.0`、`runtimeWeight=1.5`、`emotionWeight=0.3` 和 `graphWeight=0.6`。基础激活与当前线索贡献均衡，运行时工作记忆具有明显优势；情绪只作调节，图传播由于已在边和跳数上衰减，不再作为主导分量。`B_i`、`C_i`、`R_i`、`E_i` 和 `G_i` 进入顶层加权前均使用固定边界归一化到 `[0,1]`；禁止按当轮候选集做 min-max 归一化，避免同一记忆的分数因竞争者变化而漂移。

所有 cue 值归一化到 `[0,1]`；`semanticCueCoeff=2.0`、`lexicalOrTagCueCoeff=1.2`。cosine 相似度默认从 `cosineSimilarityThreshold=0.50` 开始线性映射到 `[0,1]`，低于阈值不作为语义种子。访问历史只为最多 64 个候选批量读取最近 32 次记录，并以现有 `strength`、`importance` 和累计 `accessCount` 吸收更早历史。`importanceCoeff=1.0`，与 strength 同等计入基础激活；`accessDecayExponent=-0.5`，保持 ACT-R 常用的幂律衰减。`memory_access_log` 增加 `(memory_id, created_at)` 索引。

只有最终进入 Prompt 的记忆才记录一次访问并获得强化；同一轮对话对同一记忆最多强化一次。取消当前“所有直接候选都强化”的行为，避免召回器自身形成不可控的赢家循环。访问日志写入异步持久化队列，不阻塞网络请求派发。

为避免语义索引与访问历史脱节，引入种子记忆的轻量“被考虑”奖励：HNSW 语义候选的前 5 名和图谱传播中边权重大于 0.7 的强关联节点，即使未进入最终输出，也获得小幅强度奖励（约为完整强化的 10%），不记录访问日志、不刷新最近访问时间。该奖励确保高语义相关但暂未入选的记忆仍能累积证据，同时不足以让它们仅靠被反复召回而变得不朽。

### 8. 运行时激活与离线恢复

`ActiveMemoryPool` 保存当前“脑海中”的少量记忆及其激活来源。运行期激活按来源使用不同半衰期：会话主题 60 分钟、一次联想 30 分钟、近期经历 6 小时、情绪激活 6 小时并按情绪强度缩放、未完成目标 24 小时且在目标仍有效时刷新。默认低于 `0.05` 时移出活跃池。

SQLite 保存最多 50 条激活快照，字段包括 `memoryId`、保存时激活度、来源、`lastActivatedAt` 和上下文标识。每轮交互结束后异步覆盖快照。重启时使用墙上时钟计算：

```text
restoredActivation = savedActivation * 2^(-offlineDuration / sourceHalfLife)
```

负时间差按 0 处理，异常大的时间差直接使瞬时激活失效。短时间重启可延续当时思路，长时间离线后瞬时激活自然消散；长期影响仍由强度、访问记录和最近访问时间保留。

### 9. Daydream 批次选择与巩固

Daydream 的主要触发点是夜间 `SleepCycleCoordinator`。为避免用户长期不进入夜间窗口造成堆积，当 Hippocampus 达到容量的 80%，或最旧待巩固经历已等待 24 小时时，允许在白天满足空闲、无近期待办且无模型请求运行的条件下触发补充巩固。阈值全部可配置，补充巩固仍由统一 Sleep Cycle 协调取消、暂存和提交，不恢复独立的 Legacy Daydream 提交路径。

Daydream 不按写入顺序简单截取，也不独立选择分数最高的若干条，而采用“优先级、老化保底、锚点和情景簇扩展”。候选首先必须位于 `Hippocampus`、状态为 Pending、已完整持久化、经过短暂稳定期、未被其他批次租用且不处于失败重试冷却期。

每条候选的巩固优先级由等待时间、重要程度、情绪显著性、被提及或激活次数、当前目标相关性、重复或冲突迹象以及用户明确记忆请求共同决定。等待时间提供持续增长的老化奖励，防止低显著性记忆永久饿死；明确记忆请求、任务和强烈情绪只提高处理顺序，不预先保证一定成为长期事实。

每批先选 3 至 5 个锚点：大部分来自最高优先级候选，至少一个来自等待最久的候选，并限制同一记忆簇垄断全部锚点。随后沿同一会话时间窗、临时关系一跳、同一任务或工具因果链补齐局部情景；Hippocampus 工作集扫描可通过文本、标签和时间接近性补充相似候选，但不能把不相关的表面相似文本强行并入同一事件。每个情景簇默认 4 至 8 条原始记忆，整批最多 20 条，额外长期参考记忆最多 10 条且不计入待消费数量。

选中项使用带过期时间的 lease 和版本快照。Daydream 先用确定性规则处理重复、结构化类型、隐私和失效状态，再让模型对合并、长期分区、摘要、语义标签、主题和冲突等歧义做少量判断。模型只返回提案，系统校验后生成 ChangeSet；只有 SQLite 事务、图谱变更和必要索引任务均成功提交，原始记忆才标记为 `Consolidated`。巩固不物理删除原始经历；长期记忆通过 `sourceMemoryIds` 和 `DerivedFrom` 边指向原始经历，原始内容作为可追溯 evidence 归档且不再参与普通召回。失败时释放或等待租约过期，原始经历保持可重试。

### 10. 混合建图与图谱维护

关系图采用混合建图。`DerivedFrom`、`Supersedes`、`CreatedTask` 等具有明确来源的边由代码直接建立；HNSW 为新长期记忆提供相似邻居候选，结合标签建立或强化 `Related`；同一事件中的记忆建立 `MentionedWith`；Daydream 只对候选集合中的 `TopicOf`、`ConflictsWith` 和语义歧义关系做少量模型判断。

模型只能提交包含既有节点、枚举关系、置信度和证据的关系提案，不能自由创建节点或绕过隐私与状态校验。每个 Daydream 批次最多让模型判断 8 对候选。关系表增加 `provenance`、`support_count`、`updated_at` 和 `last_reinforced_at`；规则、工具、Embedding、共现和模型来源必须可区分。每个节点最多保留 32 条普通联想边，结构性边不受该上限影响。

Daydream 清理悬空边、合并重复边，并对弱 `Related`、`MentionedWith` 边做衰减；结构性的 `DerivedFrom`、`Supersedes`、`CreatedTask` 和确认后的 `ConflictsWith` 不按普通共现衰减。新 Daydream 巩固路径必须同时维护记忆关系图和标签共现图，不能只记录标签边。

### 11. 图谱激活传播

传播最多两跳，并同时受候选总量 64 和最低传播增量 `0.08` 限制。使用优先队列维护传播前沿，但不是每次都机械选择最高激活节点。一次传播增量为：

```text
delta = sigmoid(sourceActivation)
      * edge.weight
      * edge.confidence
      * relationTypeFactor
      * 0.55^hop
      / sqrt(max(1, traversableDegree))
```

默认关系系数按联想强度为：`DerivedFrom=1.0`、`CreatedTask=0.85`、`TopicOf=0.8`、`ConflictsWith=0.7`、`Related=0.6`、`MentionedWith=0.3`。`Supersedes` 不作为普通扩散边计分，而使用定向版本解析规则：旧记忆命中时将激活完整转向新记忆，新记忆不因该关系把已取代内容带回。冲突边成对召回并显式标记，不把冲突内容当成普通事实合并。

标签共现图先从明确标签扩展最多 4 个高权重邻接标签，再通过倒排索引产生记忆种子；记忆关系图随后传播具体经历。传播记录完整路径并防止环路，同一节点从多条路径获得的增量累加但封顶为 1。

### 12. 性格、情绪与受控随机联想

传播采用约 75% 至 90% 的稳定利用和 10% 至 25% 的人格化探索。稳定分支选择当前综合激活最高的前沿；探索分支使用 Softmax 加权抽样。初始配置（v1.0）取保守值：基础探索概率为 `clamp(0.10 + 0.15 * openness, 0.10, 0.25)`，即 `explorationBaseline=0.10`、`explorationOpennessRange=0.15`；中性开放度 `openness=0.5` 时为 17.5%。温度为 `0.10 + 0.40 * openness`，中性人格时为 0.30。上述参数可配置，待离线评测与真实对话评测校准后再考虑放宽。

`openness` 提高新奇联想，`sociability` 轻度提高 `Relationship` 类型记忆以及 `MentionedWith`、`Related` 边，`initiative` 轻度提高未完成目标与任务相关边，单项偏置不超过 15%。当前情绪提高情绪一致记忆的唤起概率，但不降低事实置信度。最终最多允许 1 条探索记忆进入 Top 8，其余位置按确定性激活排序。

隐私、状态、关系合法性和任务真实性过滤始终先于随机选择，随机性不能绕过安全约束。生产使用会话级随机源；测试注入固定随机种子，保证结果可重放。

### 13. 输出、预算与可解释性

完整候选池最多 64 条，最终输出同时满足“最多 8 条”和“约 800 tokens”两个限制；非用户主动触发延续当前较小预算，最多输出 4 条。每条结果保存 `sourceChannels`、各激活分量、图谱路径、关系类型和 `exploratory` 标记。默认 Prompt 仍只注入紧凑摘要、类型、置信度和必要的冲突提示，不暴露内部数值或原始推理过程。

调试日志只记录 memoryId、分数分量、候选来源和耗时，不记录 Sensitive 内容。观测指标至少包括线索提取、Embedding、HNSW、图谱传播和精排耗时，各来源候选数量、HNSW 访问量、探索候选命中率及最终强化数量。

### 14. 迁移与降级

数据库迁移先增加索引任务、稳定 label、关系元数据、激活快照和访问日志索引。随后由后台任务分批为现有 Active 长期记忆生成向量并构建 HNSW，不能阻塞启动。迁移期间新召回以近期激活和关键词/标签索引为主；HNSW 覆盖率达到 95% 并通过校验后参与正式结果。

上线前保留旧全量召回作为 shadow 基准，只记录新旧 Top-K 差异，不向 Prompt 注入两份结果。HNSW 覆盖率达到 95% 且连续 7 天无索引损坏、无一致性校验失败后，旧全量召回退出生产代码路径；此后旧扫描仅在测试工具和离线诊断中使用。Embedding Provider 不可用、HNSW 损坏或重建时，系统降级到近期激活、关键词和标签召回，并报告索引健康状态，不在聊天热路径恢复全量扫描。

### 15. 验证策略与验收标准

单元测试覆盖线索提取、不同来源半衰期、离线恢复、ACT-R 基础激活、单轮去重强化、关系方向、两跳预算、环路、人格化概率分布、固定随机种子、隐私过滤和冲突成对召回。索引测试覆盖任意及按时间顺序插入、增量更新、墓碑删除、崩溃任务重放、模型切换、文件损坏和 SQLite 重建。

测试优先级：P0（必须先行）为索引崩溃恢复与 outbox 幂等重放、离线时长半衰期恢复、隐私过滤（Sensitive 不进 HNSW、不出现在任何候选通道）；P1（随对应阶段交付）为两跳预算与环路防护、冲突成对召回、固定随机种子可重放；其余为 P2。

使用精确全量余弦 Top-K 作为离线真值，在至少 1 万条 512 维向量上要求 HNSW `Recall@32 >= 0.95`。参考桌面环境中，预热后的 HNSW 查询、候选合并、图谱传播和精排 P95 不超过 30ms；包含查询向量生成在内的端到端本地召回 P95 不超过 100ms，GUI 线程不执行其中任何重任务。真实对话评测还需覆盖近期记忆、强印象记忆、跨措辞语义、两跳联想、矛盾记忆、长时间重启和人格化但不离题的探索。

Hippocampus 扫描需要在 50、100 和 200 条三个容量档位分别基准测试，扫描、排序和候选截断的合计 P95 应不超过 10ms，且不发生 SQLite 全库读取。查询 Embedding 生成单独计时，不用它掩盖线性扫描或数据加载开销。

代码验收要求聊天召回路径不再调用 `MemoryStore::all()`，结果规模不随长期记忆总量线性增长；任何进入 Prompt 的记忆均能解释其种子来源、激活组成和图谱路径。

观测指标除耗时与候选量外，还需覆盖：HNSW 命中转化率（Top-32 中最终进入 Top-8 的比例）、图谱传播有效性（经图谱发现且被选中的记忆占比）、探索质量（探索候选与稳定候选的平均激活分对比）。

### 16. 分阶段实施与验收

实施分四个阶段，每阶段独立验收，未通过不进入下一阶段。

**Phase 1：基础设施。** 交付 `HnswEmbeddingIndex`、`MemoryIndexWorker` 与 `memory_index_jobs` outbox、SQLite 迁移（稳定 label 映射、关系元数据、激活快照、访问日志索引）、有界 Hippocampus 工作集加载、关键词/标签倒排索引增量维护。验收：HNSW `Recall@32 >= 0.95`；预热后查询 P95 `< 10ms`；outbox 崩溃重放幂等；工作集加载不做 SQLite 全库读取。

**Phase 2：激活模型。** 交付 `ActiveMemoryPool`（瞬时激活、半衰期衰减、快照与离线恢复）、`MemoryCueExtractor`、多路种子合并（近期激活 12 + 工作集 8 + HNSW 32 + 关键词/标签 12）、不含图谱传播的 ACT-R 精排。验收：聊天召回路径不再调用 `MemoryStore::all()`；本地召回（不含查询向量生成）P95 `< 30ms`。此阶段完成即消除全量扫描，具备独立上线价值。

**Phase 3：图谱与探索。** 交付 `AssociativeActivationEngine`（两跳传播、环路防护、64 候选预算）、人格化探索（Softmax 采样、探索率 10%–25%）、含图谱分量的完整精排。验收：传播预算与环路测试通过；探索候选质量与图谱传播有效性指标开始采集并达到基本合理区间。

**Phase 4：Daydream 集成。** 交付批次选择优化（锚点、老化保底、情景簇扩展）、混合建图（规则 + Embedding 候选 + 模型复核）、Daydream 巩固后自动更新 HNSW。验收：巩固批次的老化保底与锚点约束经测试验证；HNSW 对 Active 长期记忆覆盖率 `> 95%`；shadow 对比稳定后旧召回按第 14 节条件退出。

## 澄清结论

- [2026-09-02] 召回采用符合类人表现的混合激活模型，近期记忆走独立激活池，Daydream 巩固后的长期记忆才进入 HNSW。
- [2026-09-02] 引入 HNSW 作为正式语义索引，使用 `hnswlib`；SQLite 保存权威记忆与向量，HNSW 文件作为可重建派生索引。
- [2026-09-02] 瞬时激活允许跨重启恢复，但必须按照离线时长和激活来源半衰期衰减；长期印象继续由强度和访问历史表达。
- [2026-09-02] 最终排序采用 ACT-R 启发式激活模型，不完整复刻 ACT-R；大模型不参与在线召回。
- [2026-09-02] 在线热路径只执行本地线索提取、索引查询、图谱传播和排序；首次写入不调用模型，模型只在 Daydream 阶段判断少量复杂关系。
- [2026-09-02] 关系图采用确定性规则、Embedding 候选和模型复核相结合的混合建图，新 Daydream 路径必须同时维护记忆关系图与标签图。
- [2026-09-02] 图谱传播最多两跳，受激活阈值和 64 条候选预算约束，不进行无界遍历。
- [2026-09-02] 联想平均采用约 80% 稳定选择和 20% 性格、情绪控制的探索，最终 Top 8 最多保留一条探索记忆。
- [2026-09-02] 初始候选默认来自近期激活 12 条、HNSW 32 条、关键词/标签 12 条；最终最多 8 条且约 800 tokens，所有参数可配置并由评测校准。
- [2026-09-03] 每次交互都先无模型地持久化为可追溯经历并加入近期激活视图；本地后台只补充确定性元数据和临时关系，大模型仅在 Daydream 阶段参与深度巩固。
- [2026-09-03] `Hippocampus` 负责可靠保存全部待巩固经历，`ActiveMemoryPool` 只表达当前可唤起状态；记忆退出激活池不代表已被 Daydream 消化或可以删除。
- [2026-09-03] Daydream 采用优先级、老化保底、锚点和情景簇扩展选择批次，不按 FIFO 或单纯 Top-K 独立处理记忆。
- [2026-09-03] 一次完整交互轮次对应一个 Hippocampus 经历节点，user、assistant 和 tool 内容作为可追溯子事件。
- [2026-09-03] Hippocampus 在线扫描工作集默认最多 200 条，优先使用内存有界线性扫描，不引入第二套 ANN；扫描 P95 超过 10ms 或工作集扩大到 500 条以上时重新评估，积压不得导致原始经历丢失。
- [2026-09-03] Daydream 成功后将原始经历标记为 `Consolidated` 并作为 evidence 归档，不再物理删除。
- [2026-09-03] Daydream 以夜间 Sleep Cycle 为主触发；Hippocampus 达到 80% 容量或最老项等待超过 24 小时时，可在白天空闲期补充触发。
- [2026-09-03] 激活精排采用均衡的基础/线索权重，强化运行时记忆，降低情绪、图传播和弱关系的主导性；语义阈值调整为 0.50。
- [2026-09-03] 明确区分 Hippocampus 分区（SQLite 持久队列）与 Hippocampus 工作集（内存有界视图）；交互轮次完成以 session barrier 加同步工具全部返回为判定点，异步工具结果作为延迟子事件追加。
- [2026-09-03] 初始参数取保守值：人格化探索范围调整为 10% 至 25%、温度降为 `0.10 + 0.40 * openness`、`efSearch=50`、墓碑重建阈值 30%；均可配置，待评测校准后调整。
- [2026-09-03] 在“只强化最终进入 Prompt 的记忆”基础上，为 HNSW Top-5 与强图谱关联节点保留约 10% 的轻量被考虑奖励，避免语义相关记忆的访问证据完全由最终输出垄断。
- [2026-09-03] 实施按四阶段推进（基础设施 → 激活模型 → 图谱与探索 → Daydream 集成），每阶段独立验收；Phase 2 完成即消除聊天热路径全量扫描。
