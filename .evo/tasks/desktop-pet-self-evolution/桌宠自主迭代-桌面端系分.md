...
| 仓库 | 路径 | 分支基线 | 技术栈 | 说明 |
|---|---|---|---|---|
| DesktopPet | `/Users/huang/Desktop/DesktopPet` | `master` (`1502f9f`) | C++20、Qt6、SQLite、Python/PySide6 | 单仓库；C++ 运行时与 Python launcher 共同交付 |
...
- 当前 LLM Prompt 只有宠物名和固定行为说明，没有独立的人格基线；现有 `PetPersonality` 仅服务提醒概率、时间扰动和提醒话术，不是 LLM 人格模型。
- 现有 Skill/Tool 能力可继续使用，但本阶段不改变其存储、执行和权限模型。
- Daydream 仅有设计文档，内心活动、睡前日记和模型不可见的所有者读取入口尚不存在。
...
3. 建立可中断的睡眠循环，分别完成 Daydream、私有内心活动整理和每日睡前日记。
...
- Daydream、心理摘要、每日睡前日记和多库恢复协议。
- 模型不可见、只读的所有者日记访问入口。
- 情绪消费接口与中性空实现；不重做用户未推送分支中的情绪逻辑。
...
- 情绪算法、情绪持久化或 Appraisal 流程；本分支只提供 `NullEmotionStateProvider`。
...
- [2026-08-20] 情绪系统已有未推送的分支实现；本设计不得复刻其算法或 schema，只新增独立适配接口与 `NullEmotionStateProvider`，由用户后续合并真实 Provider。
...
| 🆕 | `core/ai/integration/emotion_state_provider.*` | 仅定义情绪快照/轨迹读取契约和 `NullEmotionStateProvider`；不实现情绪计算，不落库 |
| 🆕 | `core/ai/identity/*` | 人格、关系、自我模型、证据账本和状态快照；通过接口读取可选情绪输入 |
| 🔧 | `core/ai/memory/*` | 接入事件来源、Daydream staging 和状态版本；保留现有检索能力 |
| 🆕 | `core/ai/reflection/*` | SleepCycle、Daydream、内心活动、日记、取消令牌和私有库 |
...
| 3 | 情绪接入、人格、关系与自我模型 | 以空 Provider 隔离情绪分支，管理其余多时间尺度角色状态 | 3.2、3.3 |
| 4 | Daydream、内心活动、日记与睡眠循环 | 在可取消事务中完成每日反思 | 3.2-3.4 |
...
> 依赖：§3.2 EventLedger，§3.3 Consolidation 模型角色。情绪真实实现来自未推送分支，不属于本模块。
...
状态分层：外部情绪快照（分钟至小时）、关系/行为倾向（天至数周）、人格基线（数周至数月）、身份叙事（更慢）。安全权限不属于人格，不参与学习。当前默认 Provider 始终返回中性快照，不试图近似真实情绪系统。
...
1. AgentBootstrap 仅依赖 `EmotionStateProvider`；本分支固定注入 `NullEmotionStateProvider`，不探测、读取或迁移其他分支的情绪存储。
2. PersonaProjector 和 Diary/SleepCycle 按需读取当前快照或时间轨迹；空实现分别返回版本化的中性快照和空列表，不调用模型、不派生情绪事件。
...
- N/A — 本分支不持久化 EmotionSnapshot，不创建情绪 checkpoint/schema，也不追加情绪变化事件；真实 Provider 的存储归用户未推送分支所有。
...
| `EmotionStateProvider` | 用户未推送的情绪分支 | 本分支只拥有消费契约；真实实现后续适配，禁止反向依赖其内部类型或表结构 |
...
- `currentSnapshot`：当前仅返回中性快照；调用前后无 SQLite、模型调用或事件写入，切换真实 Provider 后调用方签名保持不变。
- `trajectory`：Diary 请求当天轨迹时当前返回空列表；日记仍可依据真实事件撰写，不编造情绪经历。
...
本模块管理离线反思生命周期。Daydream 处理客观记忆，InnerThought 保存不对话展示的心理摘要，Diary 每日本地日期形成一篇自由叙事，SleepCycleCoordinator 负责触发、优先级、staging、提交和整轮回滚。
...
| 方法 | 入参字段 | 值来源 |
|---|---|---|
| `createAsync` | `sourceEventId/contextSnapshot/emotionSnapshot/privacy` | source 调用方传入；context 系统推导（来源：事件时状态）；emotion 系统推导（来源：EmotionStateProvider，当前为中性） |
| `consolidateAsync` | `sessionId/sourceCutoffSequence/maxItems` | session 系统生成；maxItems 配置默认值（来源：`sleepPolicy.maxItemsPerSession`） |
| `composeAsync` | `localDate/sourceCutoff/events/emotionTrajectory/innerThoughtRefs` | localDate 系统推导（来源：profile timezone）；events/refs 由 staging 查询；emotionTrajectory 来自 Provider，当前为空列表 |
...
4. Daydream 从待巩固区读取最多 N 条，做分类、去重、冲突检测和结构化合并，仅写各参与库的 `sleep_staged_change`。
5. 汇总当天事件、可选情绪轨迹、未解决心理摘要和已经提交的记忆，Diary role 自由决定内容；当前空 Provider 不提供轨迹，日记不得据此编造情绪经历。
...
| Daydream | `WorkingMemoryCache::cleanup` | 删除固定写 Episodic 的职责，MemoryRetriever 保持只读 |
...
- `createAsync`：一次值得反思的互动完成后生成简短私有摘要；当前可收到中性 EmotionSnapshot，但普通聊天记录、AI 调用日志和 Tool result 均不含该正文。
...
- `NullEmotionStateProvider` 始终返回中性当前快照与空轨迹，不调用模型、不访问数据库；替换测试 Provider 时人格投影和日记调用方无需改签名。
...
4. 情绪系统固定使用空 Provider；用户合并其他分支时只替换 Provider 注册，不迁移本设计中的情绪数据，因为本设计不产生该数据。
